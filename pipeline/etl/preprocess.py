...
class slope(city_task.PostgresTask):
    ...
    @property
    def update_id(self):
        return 'slope_raw:{city}'.format(self.city)
    ...
    @property
    def query(self):
        drop = ("""DROP TABLE raw.{city}_slope;""".format(city=self.city))
...
class city_center(city_task.PostgresTask):
    ...
    @property
    def update_id(self):
        return 'city_center_raw:{city}'.format(self.city)
    ...
    def query(self):
        # Parameters
        latitud = configuration.get_config().get(self.city,'latitud')
        longitude = configuration.get_config().get(self.city,'longitud')
